function this_delta = agent_agent_force(agent_num)
  global buffer;
  ...
  for otherAgent = 1:configuration.agents
    % same agent or agent inactive
    if otherAgent == agent_num || isnan(buffer(tminus(1),:)(1+2*otherAgent))
      continue
    end
    ...
    % original function definition header
    ...
    % data structure adapter variables
    AgentPosVector = this_last_position;
    AgentVelVector = this_last_delta;
    OtherPosVector = tminus1(2*agent_num-1:2*agent_num);
    VelOther = last_delta(2*agent_num-1:2*agent_num);

    % should be just about unmodified from here
    ...
    this_delta = this_delta + ForceVector;
  end
end
...
% test ForceFromAnotherAgent.m      %fprintf(fileID,'Agent: %3.0f from agent: %3.0f -> x is %8.0f , y is %8.0f\n', agent, otherAgent, Force);
