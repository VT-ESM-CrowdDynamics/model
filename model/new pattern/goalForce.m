function this_delta = goalForce(agent_num)
  ...
  G = goalArray(goalPath(goal_paths(agent_num))(current_goals(agent_num)),:); %get the goal array
  forceFromGoal = goalForce(this_last_position, [G(1),G(2)],[G(3),G(4)], velocity_upper_limits(agent_num)*configuration.dt, this_last_delta); % get goal force from function
  ...
  while(norm(forceFromGoal) < 1 && current_goals(agent_num) < length(goalPath(goal_paths(agent_num))))
    %disp("in the goal loop")
    % fprintf(fileID,'Agent:%2.0f forceGoalLOOP -> # %5.0f\n', agent, current_goals(agent_num) );
    current_goal = current_goal + 1;
    G = goalArray(goalPath(goal_paths(agent_num))(current_goals(agent_num)),:);
    % fprintf(fileID,'Agent:%2.0f G = %4.0f %4.0f %4.0f %4.0f\n', agent, G );
    forceFromGoal = goalForce(this_last_position, [G(1),G(2)],[G(3),G(4)], velocity_upper_limits(agent_num)*configuration.dt, this_last_delta);
